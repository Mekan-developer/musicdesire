...
@section('page')
    <form action="/admin/feedback/update/{{ $item->id }}" method="post">
        @csrf
        <div class="container mt-5">
            <div class="form-group mb-3">
                Текущее фото
                <img src="{{ asset('storage/feedback/' . $item->image) }}" alt="" width="50px" height="50px"
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" class="form-control modal-input" id="name" name="name"
                    placeholder="Введите имя пользователя" value="{{ $item->name }}">
            </div>
            <div class="form-group mb-3">
                <label for="content" class="modal-p">Содержание отзыва</label>
                <input type="text" class="form-control modal-input" id="content" name="content" placeholder="....."
                    value="{{ $item->content }}">
            </div>
            <div class="form-group mb-3">
                <label for="link" class="modal-p">Ссылка</label>
                <input type="text" class="form-control modal-input" id="link" name="link"
                    placeholder="https://vk.com" value="{{ $item->link }}">
            </div>
