<div>
    <p class="text-center order-track-p text-uppercase" wire:click="showApplyModal">
        {{ __('common.order_a_composition') }}
    </p>



    <!-- Modal -->
    <div class="modal fade right" wire:ignore.self id="applyModalTop" tabindex="-1" aria-labelledby="applyModalTopLabel"
        aria-hidden="true">
            ...
            <div class="modal-content ">
                ...
                <div class="modal-body">
                    <form wire:submit.prevent="addApply" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="d-flex mb-35">
                            <input type="radio" id="beep_yes" name="apply_beep"
                                class="custom-control-input form-check-input" wire:model="apply_beep" value="no">
                            <label class="custom-control-label form-check-label mr-4"
                                for="beep_yes">{{ __('common.yes') }}</label>
                            <input type="radio" id="beep_no" name="apply_beep"
                                class="custom-control-input form-check-input" wire:model="apply_beep" value="yes">
                            <label class="custom-control-label form-check-label "
                                for="beep_no">{{ __('common.no') }}</label>
                        </div>
                        ...
                        <!-- Hidden file input -->
                        {{-- <input type="file" id="file" wire:model="file" accept="audio/*" hidden> убрал accept потому-что файлы с айфона не грузят--}}
                        ...
                        <input type="text" placeholder="{{ __('common.your_name') }}" class="mb-35 modal-input"
                            name="apply_name" id="apply_name" wire:model="apply_name">
                        @error('apply_name')
                            <span>{{ $message }}</span>
                        @enderror
    ...
    @section('page-script')
        <script>
            function showApplyModal() {
                var applyModalTop = document.getElementById('applyModalTop');
                var modalInstance = bootstrap.Modal.getInstance(applyModalTop);
                if (modalInstance) {
                    modalInstance.show();
                } else {
                    modalInstance = new bootstrap.Modal(applyModalTop);
                    modalInstance.show();
                }
            }
